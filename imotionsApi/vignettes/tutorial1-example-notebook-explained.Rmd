...
# Tutorial 1: How R Notebooks Run in iMotions - An Example Notebook Explained

This article will use Skin Temperature R Notebook as an example, to explain the basics of how R Notebooks run in the 
iMotions platform, read user input parameters and generate results. The full notebook script can be found here.
...
params:
  ...
  TemperatureScaleSelection:
   ...
   input: select
   choices: ["Fahrenheit (deg F)", "Celcius (deg C)"]
output:
  html_document:
    df_print: kable
    code_folding: hide
...
these parameters will be assigned based on the user's choices in the UI. There are in total, six types of
params variables: select, checkbox, text and numeric:
...
they will not work in iMotions desktop since they need to be installed from iMotions side.
...
specified when creating analysis. The report normally includes study/analysis information, graphs/tables, algorithm
...
The properly functioning .Rmd file needs to be put in the path `C:\Program Files\iMotions\Lab_XG\RNotebooks` or for export files `C:\Program Files\iMotions\Lab_XG\RNotebooks\Exports`, in order to be able to run in iMotions desktop. Please be aware that there could be repeated notebooks of the same kind co-existing, after your editting. You can either delete the repeated ones or keep them separate by using different notebook names. Normally, you will need to provide administrator permissions to edit the files in this path.
